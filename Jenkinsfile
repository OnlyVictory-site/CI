...
pipeline {
    ...
    stages {
        ...
        stage('Build Image') {
            steps {
		            sh '/etc/init.d/docker start'
	                sh 'docker build -t yihoeun/onlyvictoryimg .'
            }
        }
        stage('Docker Login') {
            steps {
        ...
        stage('Docker Push') {
            steps {
                // sh 'docker tag onlyvictoryimg:$BUILD_NUMBER 20.228.182.157:80/onlyvictory/onlyvictoryimg:$BUILD_NUMBER'
                // sh 'docker push 20.228.182.157:80/onlyvictory/onlyvictoryimg:$BUILD_NUMBER'
                sh 'docker tag yihoeun/onlyvictoryimg yihoeun/onlyvictoryimg:latest'
                sh 'docker push yihoeun/onlyvictoryimg:latest'
            }
        }
    }
